 ...
 * - Solo rechazos cargo Cliente (+ resto aprobado) → APROBADO
 * - Otros rechazos → ENVIADO (no RECHAZADO en cabecera);
 *   Liquidaciones lo recibe / visualiza como RECHAZADO
 */
export function resolverEstadoEnvioALiquidaciones(
  danos: { cargo: string; aplica?: string }[]
): {
  estado: Extract<EstadoEstimacion, 'APROBADO' | 'ENVIADO'>;
  /** Cómo lo percibe / notifica liquidaciones RFS. */
  paraLiquidaciones: 'APROBADO' | 'RECHAZADO';
  hayRechazos: boolean;
...
/**
 * Retorno Seaboard con ítems rechazados: cabecera queda ENVIADO,
 * liquidaciones lo trata como RECHAZADO.
 */
